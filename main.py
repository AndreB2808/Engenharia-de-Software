...
def getlab(nome: str, opcao: str) -> str:
    ...
    if opcao == "1":
        ...
        if sala in salas_reservadas:
            print("Essa sala já está reservada!")
            # retornar para input de informar a sala
        elif int(sala) not in salas:
            print("Essa sala não existe!")
            # retornar para input de informar a sala
        else:
            loading_fake(3)
            salas_reservadas.append(int(sala))
            reservadores.append(nome)
            print(f"Reserva realizada com sucesso para a sala {sala}!")
            print(txt_reserva())
        ...
        loading_fake(2)
        if nome in reservadores:
            index = reservadores.index(nome)
            print(f"Você possui uma reserva para a sala {salas_reservadas[index]}.")
            print(txt_reserva())
        else:
        ...
        if nome in reservadores:
            index = reservadores.index(nome)
            salas_reservadas.pop(index)
            reservadores.pop(index)
            print(f"Reserva encontrada para a sala {salas_reservadas[index]}.")
            escolha = input("Deseja prosseguir com o cancelamento? (s/n): ")
            if escolha == "s":
                print("Reserva cancelada!")
                # remover a reserva da sala e do reservador
            else:
